...
parser.add_argument("-b", "--redhat-bucket", dest='buckets', action="append", default=[], help="Redhat bucket(s) to scrape")
...
for bucket in args.buckets:
    ...
    while not done:
        ...
        url = "{RHAPI_BASE_URL}/packages/cset/{bucket}/arch/x86_64".format(RHAPI_BASE_URL=RHAPI_BASE_URL, bucket=bucket)
        ...
        if do_download:
            ...
            while num_retries < MAX_RETRIES:
                try:
                    ...
                    num_retries += 1
                    log("Error {status} while downloading {pn}, retrying attempt #{num_retries}/{MAX_RETRIES}".format(
